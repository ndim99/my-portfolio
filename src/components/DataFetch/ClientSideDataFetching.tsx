...
export function ClientSideDataFetching({
  ...
  return (
    <div className="w-full 2xl:p-4 p-3 bg-gray-100 dark:bg-gray-900 rounded-lg flex flex-col">
      {loading ? (
        <div className="flex items-center justify-center fontSizeFromXl font-semibold text-primary-colors">
          Loading...
        </div>
      ) : error ? (
...
export default function ClientSideData({ tokenId }: ClientSideDataProps) {
  const [data, setData] = useState<TokenData | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  ...
  // Reusable Row component for cleaner structure
  const DataRow = ({
    label,
    value,
  }: {
